...
async function fetchFromAPI(url: string, params: Record<string, string>): Promise<any> {
...
async function fetchFromNewsApi(filters: NewsFilters): Promise<Article[]> {
  ...
  return data.articles.map((article: NewsApiArticle): Article => ({
...
async function fetchFromGuardian(filters: NewsFilters): Promise<Article[]> {
  ...
  return data.response.results.map((article: GuardianApiArticle): Article => ({
    ...
    newsapi: [
      {
        ...
        description: "Latest developments in technology are transforming how we live and work in unprecedented ways.",
      ...
      },
      {
        ...
        description: "Financial markets continue to demonstrate stability despite ongoing global challenges.",
    ...
    guardian: [
      {
        ...
        description: "Countries around the globe are implementing new policies to address climate change challenges.",
        url: "https://example.com/environment-news",
        imageUrl: "https://images.unsplash.com/photo-1569163139382-de56a4d3154a?w=800",
        source: "The Guardian",
        category,
    ...
    nytimes: [
      {
        ...
        description: "New cultural movements are influencing art, entertainment, and social interactions across demographics.",
...
const newsSlice = createSlice({
  ...
  reducers: {
    ...
    toggleFavorite: (state, action: PayloadAction<{ type: FavoriteType; article: Article }>) => {
      const { type, article } = action.payload;
      const favoriteKey = `favorite${type.charAt(0).toUpperCase() + type.slice(1)}` as keyof NewsState;
